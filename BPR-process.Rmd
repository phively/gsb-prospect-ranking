...
dat %>%
  ...
  geom_density(alpha=.5, aes(color=Giving.Status, fill=Giving.Status)) +
  # Reference lines for the group means
  geom_vline(xintercept = dat %>% filter(Giving.Status=="Nondonor") %>% select(Booth.ClassYr.or.RecYr) %>% unlist() %>% mean(),
             linetype="dashed", color="pink", alpha=.75) +
  geom_vline(xintercept = dat %>% filter(Giving.Status=="Donor") %>% select(Booth.ClassYr.or.RecYr) %>% unlist() %>% mean(),
